...
class DataOutlier():
    ...
    def getOneDetectionResult(self, data, config):
        ...
        self.anomaly_score_method = config['anomaly_score_method']
        ...
        if 'anomaly_score_param'in config:
            self.anomaly_score_param = config['anomaly_score_param']
        else:
            self.anomaly_score_param = None
    ...
    def getModel(self, data_col):
        ...
        if self.algorithm == 'SR':
            ...
            model = LocalOutlierFactor(n_neighbors=self.args['LOF_neighbors'], novelty=False, 
    ...
    def getIndexList(self, data_col, model):
        ...
        if self.algorithm == 'SR':
            ...
            score = - 1.0 * model.negative_outlier_factor_
        else:
            score = - 1.0 * model.score_samples(data_col)
            
        indexes = anomaly_threshold.get_anomaly_indexes(score, self.anomaly_score_method, self.anomaly_score_param)
        
        print(f"Anomaly Indexes Counts: {len(indexes)}")
