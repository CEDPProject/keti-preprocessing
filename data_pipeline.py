...
def pipeline(data, module_list, feature_name = None, expected_types=None):
    """ 
    ...
    Args:
        ...
        expected_types (dict, optional): 컬럼별 기대 타입 정의. {'column_name': type} 형식. 예: {'temperature': float, 'co2': int}
    ...
    Result = pipeline(input_df, pipeline_list, feature_name="temperature", expected_types={"temperature": float})
    ...
    for module in module_list:
        ...
        if module_name =="data_outlier":
            ...
            from clust.transformation.general import split_interface
            data = split_interface.get_data_result(split_method, data, split_param)
            
        elif module_name == 'data_selection':          
            from clust.transformation.general import select_interface
    ...
    if expected_types:  
        data = enforce_column_types(data, expected_types)

        
    return data

def enforce_column_types(df, expected_types):
    for col, expected_type in expected_types.items():
        if expected_type == int:
            df[col] = df[col].apply(lambda x: int(round(x)) if pd.notna(x) else pd.NA).astype('Int64')
        else:
